...
class TestConfig(unittest.TestCase):
    ...
    def test_load_config_successfully(self):
        ...
        """
        # This test now verifies the structure for multiple profiles.
        config = load_config(str(self.config_path))
        self.assertIsInstance(config, Config)
        self.assertEqual(len(config.main.target_profiles), 1)
        self.assertEqual(config.main.target_profiles[0].name, "testuser")
        self.assertEqual(config.main.target_profiles[0].num_posts, 10)
    ...
    def test_path_expansion(self):
        ...
        NOTE: This test is now less relevant as path expansion is handled
        dynamically inside the pipeline. We'll test a simple case.
        """
        from igscraper.config import expand_paths
        config = load_config(str(self.config_path))
        expand_paths(config, {"target_profile": "testuser"})
